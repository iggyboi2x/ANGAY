...
import { TrendingUp, Users, Package, AlertCircle, ArrowUpRight, ArrowDownRight, Building2, MapPin } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [profiles, foodbanks, barangays] = await Promise.all([
        supabase.from('profiles').select('id', { count: 'exact', head: true }),
        supabase.from('foodbanks').select('id', { count: 'exact', head: true }),
        supabase.from('barangays').select('id', { count: 'exact', head: true })
      ]);
      const totalUsers = (profiles.count || 0) + (foodbanks.count || 0) + (barangays.count || 0);
      ...
      setStats([
        { label: 'Total Active Users', value: totalUsers.toLocaleString(), change: null, icon: Users, color: 'blue' },
        { label: 'Pending Tasks', value: (pendingReports || 0).toString(), change: null, icon: AlertCircle, color: 'red' },
        { label: 'Total Units Moved', value: totalItems.toString(), change: null, icon: Package, color: 'green' },
      ]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {stats.map((stat, idx) => (
            <div key={idx} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
              ...
              <h3 className="text-2xl font-black text-[#1A1A1A] mt-1">{loading ? '...' : stat.value}</h3>
            </div>
          ))}
        </div>
